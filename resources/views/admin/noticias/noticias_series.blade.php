...
<div class="container">
	<div class="row">		
		<div class="col-md-11 col-md-offset-1">
				{!! Form::open(['route' => 'admin.noticias2.store', 'method' => 'POST','files' => true]) !!}
					<h3><i class="fa fa-pencil"> Registrar Noticia de Series </i></h3>
					<hr>
					<br>
					<div class="form-group">
						{!! Form::label('series', 'Series') !!}
                        {!! Form::select('series[]', $series2, $my_series,['id' => 'series', 'class' => 'form-control select-pelis', 'multiple', 'required']) !!}&nbsp;
                    </div>
					<div class="form-group">				
                ...
                <tbody id="datos_noticia_peli">
                  @foreach($noticias as $noticia)
                    <tr>
                      <td>{{ $noticia->id }}</td>   
                      <td>                       
                         <img src="{{ asset('img/noticias_series/' . $noticia->foto) }}" alt="" style="min-height: 30px;max-height: 30px;">
                      </td>
                      <td>{{ $noticia->titulo }}</td>  
                      <td>{{ $noticia->descripcion }}</td>   
                      <td><a href="{{ route('admin.noticias2.destroy', $noticia->id) }}" class="btn btn-danger"> <i class="fa fa-trash" aria-hidden="true"> Delete</i></a></td>
                    </tr>
                  @endforeach
                </tbody>
              </table>
